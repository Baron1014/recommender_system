...
import wandb
import numpy as np
from dataaccessframeworks.read_data import get_douban, training_testing, user_filter, training_testing_XY
...
def main():
    ...
    fm(X_train, y_train, X_test, y_test, test_index, users, books)

    # 取得加上使用者未評分的sample假資料
    include_fake = get_norating_data(filter_data[:, :3])
    training_data,  testing_data = training_testing(include_fake)
    # 5. BPR-MF
    #bpr_mf(training_data, testing_data, users, books)
    # 6. BPR-FM
    #bpr_fm(training_data, testing_data, users, books)
    # 7. GBDT + LR
    # gbdt_lr(X_train, y_train, X_test, y_test)
    # 8. xgboost + LR
    #execute_xgb_lr(X_train, y_train, X_test, y_test, test_index, users, books)
    ###################################################################
    ## NN-based RecSys Methods
    ###################################################################
    # 取得user及items feature map 
    users_dict, items_dict, features = get_feature_map(data, 'user_book')
    dataframe = generate_with_feature(training_data, users_dict, items_dict, init_col=["user", "book", "rating"])
    test_dataframe = generate_with_feature(testing_data, users_dict, items_dict, init_col=["user", "book", "rating"])
    # 1. FM-supported Neural Networks
    fnn(dataframe, test_dataframe, test_index, users, books)
    # 2. Product-based Neural Networks
    ipnn(dataframe, test_dataframe, test_index, users, books)
    opnn(dataframe, test_dataframe, test_index, users, books)
    # 3. Convolutional Click Prediction Model 
    ccpm(dataframe, test_dataframe, test_index, users, books)
    # 4. neumf
    # 5. Wide&Deep
    wd(dataframe, test_dataframe, test_index, users, books)
    # 6. Deep Drossing
    dcn(dataframe, test_dataframe, test_index, users, books)
    # 7. Neural Factorization Machine
    nfm(dataframe, test_dataframe, test_index, users, books)
    # 8. Deep Factorization Machine
    deepfm(dataframe, test_dataframe, test_index, users, books)
    ...
    afm(dataframe, test_dataframe, test_index, users, books)
    # 3. xDeepFM
    xdeepfm(dataframe, test_dataframe, test_index, users, books)
    # 4. Deep Interest Network
    #din(dataframe, test_dataframe, test_index, users, books)


    ###################################################################
    ## Ensemble Methods
    ###################################################################
    EnsemblemModel(dataframe, test_dataframe, y_test, test_index, users, books)
...
def din(train_df, test_df, test_index, users, movies, watch_history = ['book', 'book_publisher'], target="rating"):
    run = wandb.init(project=config['general']['douban'],
                        entity=config['general']['entity'],
                        group="DIN",
                        reinit=True)
...
def fnn(dataframe, testing_data, test_index, users, movies):
    run = wandb.init(project=config['general']['douban'],
                        ...
                        group="FNN",
                        reinit=True)
    deer, _ = DeepCTRModel(sparse=['user', 'book', 'user_location', 'book_author', 'book_publisher', 'user_group'],
                        dense=['book_year'],
                        y=['rating'])
